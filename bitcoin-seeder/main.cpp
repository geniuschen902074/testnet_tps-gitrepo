...
extern "C" void* ThreadSeeder(void*) {
  ...
  MSG *buff_send = new MSG()/*malloc(1 * sizeof(MSG))*/;
  void *buff_recv = malloc(1 * sizeof(MSG));
  ...
  do {
    ...
    */
    ...
    int res = recv(clientfd, buff_recv, sizeof(MSG), 0);
    switch( ((MSG *)buff_recv)->op ){
        ...
        case REGISTER_BTCADDR:{
            ...
            strcpy(addr, ((MSG *)buff_recv)->message);
            BTC_ADDR.push_back(addr);
            break;
        }
        case REQUEST_BTCADDR:{
            strcpy(buff_send->message, BTC_ADDR[rand() % (BTC_ADDR.size())]);
            send(clientfd, buff_send, sizeof(MSG), 0);
            break;
        }
